...
<head>
    <meta charset="UTF-8">
    <title>Tambah Berita - HMTI</title>
...
<body class="bg-gray-100 text-gray-900">
    ...
    <div class="relative h-72 flex items-center justify-center text-white">
        ...
        <h1 class="text-4xl font-bold z-10">Tambah Berita</h1>
    </div>

    <!-- Form Content -->
    <div class="max-w-2xl mx-auto px-4 py-16">
        <form action="{{ route('berita.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl shadow-lg space-y-6">
            @csrf

            <div>
                <label for="title" class="block text-gray-700 font-bold mb-2">Judul</label>
                <input type="text" name="title" id="title" required class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div>
                <label for="content" class="block text-gray-700 font-bold mb-2">Konten</label>
                <textarea name="content" id="content" rows="6" required class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400"></textarea>
            </div>

            <div>
                <label for="image" class="block text-gray-700 font-bold mb-2">Gambar (Opsional)</label>
                <input type="file" name="image" id="image" accept="image/*" class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div class="flex justify-between">
                <a href="{{ url('/berita') }}" class="inline-block bg-gray-500 hover:bg-gray-600 text-white py-2 px-6 rounded-lg transition">
                    Kembali
                </a>
