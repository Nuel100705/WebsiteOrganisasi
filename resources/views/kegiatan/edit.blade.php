...
<head>
    <meta charset="UTF-8">
    <title>Edit Program Kerja - HMTI</title>
...
<body class="bg-gray-100 text-gray-900">
    ...
    <div class="relative h-72 flex items-center justify-center text-white">
        ...
        <h1 class="text-4xl font-bold z-10">Edit Program Kerja</h1>
    </div>

    <!-- Form Content -->
    <div class="max-w-xl mx-auto px-4 py-16">
        <form action="{{ route('kegiatan.update', $program->id) }}" method="POST" class="bg-white p-8 rounded-2xl shadow-lg">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title" value="{{ old('title', $program->title) }}" required
                       class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 font-bold mb-2">Deskripsi</label>
                <textarea name="description" rows="5" required
                          class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('description', $program->description) }}</textarea>
            </div>

            <div class="flex justify-between">
                <a href="{{ url('/kegiatan') }}" class="bg-gray-500 hover:bg-gray-600 text-white py-2 px-6 rounded-lg transition">
                    Kembali
                </a>
