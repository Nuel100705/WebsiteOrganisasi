...
<head>
    <meta charset="UTF-8">
    <title>Edit Struktur - HMTI</title>
...
<body class="bg-gray-100 text-gray-900">
    ...
    <div class="relative h-72 flex items-center justify-center text-white">
        ...
        <h1 class="text-4xl font-bold z-10">Edit Struktur</h1>
    </div>

    <!-- Form Content -->
    <div class="max-w-2xl mx-auto px-4 py-16">
        <form action="{{ route('struktur.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl shadow-lg">
            @csrf
            @method('PUT')

            <div class="mb-6">
                <label class="block text-gray-700 font-bold mb-2">Nama</label>
                <input type="text" name="name" value="{{ old('name', $item->name) }}" class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400" required>
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 font-bold mb-2">Jabatan</label>
                <input type="text" name="role" value="{{ old('role', $item->role) }}" class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400" required>
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 font-bold mb-2">Divisi (jika ada)</label>
                <input type="text" name="division" value="{{ old('division', $item->division) }}" class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div class="mb-8">
                <label class="block text-gray-700 font-bold mb-2">Foto (opsional)</label>
                <input type="file" name="photo" class="w-full border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div class="flex justify-between">
                <a href="{{ url('/struktur') }}" class="inline-block bg-gray-500 hover:bg-gray-600 text-white py-2 px-6 rounded-lg transition">
                    Kembali
                </a>
